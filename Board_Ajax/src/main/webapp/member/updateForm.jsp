    ...
    <label>
      ...
      <span id="showImage">
        ...
        <c:if test='${!empty memberinfo.memberfile}'>
          ...
          <input type="hidden" name="check" value="${memberinfo.memberfile}"><%-- 파일이 있는데 변경하지 않는 경우 그대로 유지 --%>
        </c:if>
        <img src="${src}" width="20px" alt="profile">
      </span>
      <%--accept : 업로드할 파일 타입을 설정한다.
          <input type="file" accept="파일 확장자|audio/*|video/*|image/*">
          (1) 파일 확장자는 .png, .jpg, .pdf, .hwp 처럼 (.)으로 시작되는 파일 확장자를 의미한다.
              eg) accept = ".png, .jpg, .pdf, .hwp"
...
//처음 화면 로드시 보여줄 이메일은 이미 체크 완료된 것이므로 기본 checkmail=true이다.
 let checkmail=true;
 $("input[name=email]").on('keyup',
		 function() {
	 		...
	 		//\w+는 [A-Za-z0-9_]를 1개 이상 사용하라는 의미다.
 ...
 $('input[type=file]').change(function(event){
	 ...
	 if (pattern.test(filename)) {
		 ...
		 //DataURL 형식(접두사 data:가 붙은 URL이며 바이너리 파일을 Base64로 인코딩하여 ASCII 문자열 형식으로)
		 //파일을 읽어온다. (참고-Base64 인코딩은 바이너리 데이터를 Text 로 변경하는 Encoding이다.)
		 //읽어온 결과는 reader 객체의 result 속성에 저장된다.
		 //event.target.files[0] : 파일리스트에서 첫번째 객체를 가져온다.
