...
<%--
1. 검색어를 입력한 후 다시 memberList.net으로 온 경우 검색 필드와 검색어가 나타나도록 한다.

2. 검색 필드를 변경하면 검색어 입력창에 placeholder 나타나도록 한다.
   예로 아이디를 선택하면 placeholder로 "아이디 입력하세요"라고 나타난다.
   예로 이름을 선택하면 placeholder로 "이름 입력하세요"라고 나타난다.
   예로 나이를 선택하면 placeholder로 "나이 입력하세요"라고 나타난다.
   예로 성별을 선택하면 placeholder로 "여 또는 남 입력하세요"라고 나타난다.
   
3. 검색 버튼 클릭시 다음을 체크한다.
   1) 검색어를 입력하지 않은 경우 "검색어를 입력하세요" 라고 대화상자가 나타나게 한다.
   2) 나이는 2자리 숫자가 아닌 경우 "나이는 형식에 맞게 입력하세요(2자리 숫자)" 라고 대화상자가 나타나게 한다.
   3) 성별은 "남" 또는 "여"가 아닌 경우 "남 또는 여를 입력하세요" 라고 대화상자가 나타나게 한다.
   
4. 회원 목록의 삭제를 클릭한 경우
   confirm("정말 삭제하시겠습니까?")를 실행해서 취소를 클릭하면 "memberDelete.net"로 이동하지 않는다.
 --%>
<script>
$(function() {
	...
	if (selectedValue != '-1')
		...
		selectedValue=0; //선택된 필드가 없는 경우 기본적으로 아이디를 선택하도록 한다.
	
	//검색 후 selectedValue값에 따라 placeholder가 나타나도록 한다.
