...
public class BoardModifyProcessAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			//글쓴이인지 확인하기 위해 저장된 비밀번호와 입력된 비밀번호를 비교한다.
			...
			//BoardBean 객체에 글 수정 폼에서 입력 받은 정보들을 저장한다.
			...
			if (check != null) {//파일첨부를 변경하지 않으면
				...
				//업로드된 파일의 시스템 상에 업로드된 실제 파일명을 얻어 온다.
				String filename = multi.getFilesystemName("board_file");
				boarddata.setBoard_file(filename);
			}
			
			
			//DAO에서 수정 메소드 호출하여 수정한다.
			...
			if (!result) {
				...
				//수정한 글 내용을 보여주기 위해 글 내용 보기 페이지로 이동하는 경로를 설정한다.
