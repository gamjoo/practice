...
public class FileUpload extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
	  ...
	  //(1bit * 1024 = 1KB => 1KB * 1024 = 1MB)파일 최대 크기를 10MB로 지정한다.
	  String name="", subject="", systemName1="", systemName2="",
			 origfileName1="", origfileName2="";
	  
	  try {
	  /*업로드를 담당하는 부분이다.
	    1. 인자1(request) : form 태그를 통해 가져온 값을 얻기 위해 request객체를 전달해준다.
	    2. 인자2(uploadPath) : 업로드될 파일을 관리할 위치
	    3. 인자3(size) : 업로드 할 파일의 최대 크기, 지정 크기보다 크면 Exception 발생
	   ...
	   */
		...
		//중복된 파일명의 경우 자동으로 파일명 맨뒤에 숫자를 붙여 처리한다.
