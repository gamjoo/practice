...
*표현언어(Expression language : EL)
 -JSP 스크립트를 대신하여 속성 값들을 좀 더 편리하게 출력하기 위해 제공되는 언어
 -톰캣에서 제공하기 때문에 따로 설정할 필요가 없다.
 -형식) ${}
 
 1.표현식 태그 <%=article.getId()%> //article이 자바빈인 경우
   =>${article.id} 또는 ${article["id"]} 또는 ${article['id']}
   
 2.<%=reuqest.getParameter("name")%>
   =>${param.name}
 ...
 6.${hoho}
   이 경우에는 pageContext -> request -> session -> application 객체의
   영역 순으로 'hoho'이름의 attribute를 찾는다.
 --%>
...
application.setAttribute("id", "application/context");
%>
<html>
<head>
