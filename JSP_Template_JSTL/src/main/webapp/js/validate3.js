  ...
  let idcheck_value = ''; //id 중복검사시 값
	$("#myform").submit(function() {
		...
		if ($.trim(id.val()) == "") {
			...
			//회원가입 폼과 정보 수정 폼에서 동시에 사용할 js이다
			//회원가입 폼에서만 사용할 문장들이다
			//정보수정 폼에서는 아이디를 수정하지 않기 때문에 필요없는 부분이다.
		if(!id.prop("readOnly")) {
			console.log(id.prop("readOnly"))
			const submit_id_value=$.trim(id.val())
			if(submit_id_value != idcheck_value) {
			//submit 당시 아이디값과, 아이디 중복검사에 사용된 아이디를 비교한다
				alert("ID 중복검사를 하세요");
				$("#opener_message").text("");
				return false;
			}
			...
			//아이디 중복 검사를 했지만, 사용중인 아이디인 경우에는 submit시 경고창이 나타난다.
